...
def test_demo_script_runs_and_has_real_page_read_step() -> None:
    ...
    assert "本地浏览器读取页面信息 + 页面识别 v3" in output
    ...
    if "[FAIL] PAGE_INFO_READ_FAILED" in output:
        ...
        assert "步骤 3/3：页面识别（基于 title / URL）" in output
        assert "当前页面标题" in output
        assert "当前页面 URL" in output
        assert "当前是否识别为 Amazon 页面" in output
        assert "当前是否识别为 ABA 页面" in output
        assert "未执行真实 ABA 抓取、真实点击导出、真实解析、真实飞书写入" in output
